...
class Blockchain(object):
  def __init__(self):
    self.chain = [] # ブロックチェーンを収めるための最初の空のリスト
    self.current_transactions = [] # トランザクションを収めるための空のリスト
  ...
  def resolve_conflicts(self):
    '''
    これがコンセンサスアルゴリズムである。
    ネットワーク上最も長いチェーンで自らのチェーンを置き換えることでコンフリクトを解消する。
